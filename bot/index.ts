...
import { getCommandResponse, isCommandMessage } from './script/commandHelper';
...
client.on('message', async (message: Message) => {
	...
	if (!checkWhitelistStatus(message.from)) {
		...
		logWarning('Received message from unregistered user:', `(${contact.pushname})`, message.from, message.body);

		await message.reply(`🤖 Sorry, you are not allowed to use to use the Boy. Message ${process.env.OWNER_ID} for an access key 😌👌.`);
