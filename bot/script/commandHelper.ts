...
import { SupportedLanguage, disableAudioResponse, enableAudioResponse } from "./ttsHelper";
import { getAiImageBase64 } from "./imageHelper";
import { getMessageMediaFromBase64, getMessageMediaFromUrl } from "./whatsappHelper";
...
const registerCommand = async (commandData: CommandData): Promise<CommandResponse> => {
...
const contextCommand = async (commandData: CommandData): Promise<CommandResponse> => {
...
const getContextCommand = async (commandData: CommandData): Promise<CommandResponse> => {
...
const clearContextCommand = async (commandData: CommandData): Promise<CommandResponse> => {
...
const videoUrlCommand = async (commandData: CommandData): Promise<CommandResponse> => {
	...
	try {
		...
		return {
			type: CommandResponseType.Text,
			message: `Something went wrong generating a video from an url 😩 (${(error as Error).message})`,
		}
	}
}

const videoCommand = async (commandData: CommandData): Promise<CommandResponse> => {
	...
	try {
		...
		return {
			type: CommandResponseType.Text,
			message: `Something went wrong generating a video 😩 (${(error as Error).message})`,
		}
	}
}


const imageCommand = async (commandData: CommandData) : Promise<CommandResponse> => {
	...
	if('image' in imageBase64) {
		...
		return {
			type: CommandResponseType.Media,
			media: getMessageMediaFromBase64('image/jpeg', imageBase64.image.split(',')[1] as string),
			message: `*_"${`${text}`.trim()}"_*`
		}
	}
	...
	return {
		type: CommandResponseType.Text,
		message: imageBase64.error || 'Something went wrong generating an image 😩',
	}
}

const enableAudioCommand = async (commandData: CommandData): Promise<CommandResponse> => {
	...
	if (!language) {
		...
		return {
			type: CommandResponseType.Text,
			message: `No language given 🤔. Supported languages are:\n*${Object.values(SupportedLanguage).join(', ')}*`,
		}
	}

	const isoCode = language.trim();

	if (!Object.values(SupportedLanguage).includes(`${isoCode}`.toLowerCase() as SupportedLanguage)) {

		return {
			type: CommandResponseType.Text,
			message: `Language *${`${isoCode}`.toUpperCase()}* is not supported 🚫. Supported languages are:\n*${Object.keys(SupportedLanguage).join(', ')}*`,
		}
	}
...
const disableAudioCommand = async (commandData: CommandData): Promise<CommandResponse> => {
...
export const getCommandResponse = async (message: Message) => {
	...
	const commandResponse: CommandResponse = await commandHandle.handler(commandData);
