...
const conversationFilePath = createFileIfNotExists(
	`${process.cwd()}${process.env.CONVERSATIONS_PATH}`
);

const promptMapPath = createFileIfNotExists(
	`${process.cwd()}${process.env.PROMPTS_PATH}`
);
